...
class FleetManager:
    ...
    def __init__(self, console: Console | None = None):
        ...
        self.fleet_config_path = (
            self.home_dir / ".config" / "mcp-manager" / "fleet.json"
        )
    ...
    def sync_fleet_configuration(self) -> dict[str, Any]:
        ...
        for hostname, node_data in fleet_data["nodes"].items():
            ...
            try:
                ...
                sync_result["project_standards"] = self._sync_project_standards_to_node(
                    node_data
                )

                # Sync Claude agents
                sync_result["claude_agents"] = self._sync_claude_agents_to_node(
                    node_data
                )

                # Sync Python environment
                sync_result["python_environment"] = (
                    self._sync_python_environment_to_node(node_data)
                )
    ...
    def audit_fleet_compliance(self) -> dict[str, Any]:
        ...
        for hostname, node_data in fleet_data["nodes"].items():
            compliance = {
                "ubuntu_version": node_data.get("ubuntu_version", "").startswith(
                    requirements["ubuntu_version"]
                ),
                "python_version": node_data.get("python_version", "").startswith(
                ...
                "tools_installed": self._check_tools_on_node(
                    node_data, requirements["required_tools"]
                ),
    ...
    def deploy_configuration_update(
        ...
        for hostname, node_data in fleet_data["nodes"].items():
            ...
            try:
                if config_type == "mcp_server":
                    ...
                    success = self._deploy_project_template_to_node(
                        node_data, config_data
                    )
    ...
    def _load_fleet_config(self) -> dict[str, Any]:
        ...
        if self.fleet_config_path.exists():
            with open(self.fleet_config_path) as f:
                return json.load(f)
        else:
    ...
    def _get_ubuntu_version(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["lsb_release", "-r", "-s"], capture_output=True, text=True, check=True
            )
    ...
    def _check_tools_on_node(
        self, node_data: dict[str, Any], required_tools: list[str]
    ) -> bool:
    ...
    def _deploy_mcp_server_to_node(
        self, node_data: dict[str, Any], config_data: dict[str, Any]
    ) -> bool:
        """Deploy MCP server configuration to a node."""
        # Implementation for deploying MCP server
        return True

    def _deploy_project_template_to_node(
        self, node_data: dict[str, Any], config_data: dict[str, Any]
    ) -> bool:
    ...
    def _deploy_claude_agent_to_node(
        self, node_data: dict[str, Any], config_data: dict[str, Any]
    ) -> bool:
