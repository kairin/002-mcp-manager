...
from .models import MCPServer, ServerConfig, ServerType, ServerStatus, GlobalConfig
...
class MCPManager:
    ...
    def init_global_config(self, force: bool = False) -> None:
        ...
        if self.claude_config_path.exists() and not force:
            self.console.print("[yellow]Global configuration already exists. Use --force to overwrite.[/yellow]")
            return
        ...
        self.console.print(f"[green]✅ Global configuration initialized at {self.claude_config_path}[/green]")
    ...
    def init_project_config(self, force: bool = False) -> None:
        ...
        if project_config_path.exists() and not force:
            self.console.print("[yellow]Project configuration already exists. Use --force to overwrite.[/yellow]")
            return
        ...
        self.console.print(f"[green]✅ Project configuration initialized at {project_config_path}[/green]")
    ...
    def add_server(
        ...
        try:
            ...
            config_path = self.claude_config_path if global_config else Path.cwd() / ".claude.json"
    ...
    def audit_configurations(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        audit_results = {
            "global_config": {
                ...
                "servers": len(self.list_servers()) if self.claude_config_path.exists() else 0,
                        ...
                        if project_config.exists():
                            try:
                                ...
                                audit_results["project_configs"][project_name] = {
                                    ...
                                    "status": "needs_migration" if config.get("mcpServers") else "empty"
                                }
                            except Exception as e:
    ...
    def update_server(self, name: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        return {
            "updated": False,
            "details": "Update functionality not yet implemented"
        }
                        ...
                        if project_config_path.exists():
                            try:
                                ...
                                if project_servers:
                                    ...
                                    if create_backup:
                                        backup_path = project_config_path.with_suffix(".json.backup")
